class BinaryTreeNode
{
    ...
    func getMaxDepth(treeNode: BinaryTreeNode?) -> Int
    {
        ...
        let total = max(getMaxDepth(treeNode: node.leftNode),
            getMaxDepth(treeNode: node.rightNode)) + 1
        return total
    }
    func isBinarySearchTree(treeNode: BinaryTreeNode?) -> Bool
    {
        ...
        if node.rightNode != nil && node.content >=
            (node.rightNode?.content)!
        {
            return false
        }
        
        return isBinarySearchTree(treeNode: node.leftNode) &&
            isBinarySearchTree(treeNode: node.rightNode)
    }
}
